...
With all that has been seen so far, let's create a new user on the destination machines called "ansible", configure the user to be able to sudo, and provide the user with a password-less connection. 

Thinking about how to send the user password through the playbook without showing a plain text password (https://docs.ansible.com/ansible/latest/user_guide/playbooks_vault.html#single-encrypted-variable).
